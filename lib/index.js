		...
		return new Promise((resolve, reject) => {
			...
			if (oldInfo) {
				...
				if (oldInfo.Id !== info.Id) {
					...
					changed = true;
					return removeContainer(oldInfo).then(() => {
			...
			return reject("update container did not add or remove a container");
		});
	};
					...
					data.on("data", chunk => {
						...
						async.each(
							...
							(line, done) => {
								try {
									if (line) {
										console.log("processDockerEvent: %s", line);
										return processDockerEvent(JSON.parse(line)).then(() => {
											console.log("after update container");
											done();
										})
										.catch((err) => {
